# Dashboard / receiver — running the board & port troubleshooting
...
```bash
npx @qin16778/tracelink@latest board     # recommended
node bin/tracelink.mjs board             # from a clone of the repo root
```
...
> **`npx` stale-cache gotcha:** plain `npx @qin16778/tracelink board` may execute an older cached copy. **Always pass `@latest`** (`npx @qin16778/tracelink@latest board`) to force the current version.
...
import { debugLogPlugin } from '@qin16778/tracelink/receiver/vite';
export default defineConfig({ plugins: [debugLogPlugin()] });
```
```typescript
// Non-Vite (Next.js / plain Node): standalone node:http receiver, default :5174
import { startReceiverServer } from '@qin16778/tracelink/receiver/http';
...
| `GET` | `/__debug_log/stream` | SSE: replay current buffer as `event: log` frames, then push each new log live; `: heartbeat` ~every 15s | `200` `text/event-stream` |
| `GET` | `/__debug_log/scopes` | Read `{ enabled, known }` (authoritative enabled scopes + all scopes seen) | `200` JSON |
| `POST` | `/__debug_log/scopes` | Set authoritative enabled scopes (body `{ enabled }`); persists to `.tracelink/scopes.json` | `204` |
...
<project-root>/.tracelink/scopes.json      # persisted authoritative scope config
```
...
## The port footgun (read this before "the board is broken")

- **The receiver OWNS `5174`.** The embedded single-file dashboard at `/__debug_log/ui` is served **by the board itself** on `5174`. As a consumer, you almost always just need the board on `5174` — that's it.
